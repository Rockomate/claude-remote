...
export default function Settings() {
  ...
  const selectProject = (p: ProjectInfo) => setWorkDir(p.path);
    ...
    <div className="app-layout">
      <div className="main-panel">
        <div className="chat-header">
          <h1>Settings</h1>
          <button className="icon-btn" onClick={() => navigate('/')} style={{ fontSize: 13, padding: '6px 12px' }}>&larr; Back</button>
        </div>
        <div style={{ flex: 1, overflow: 'auto', padding: 16 }}>
          <div className="config-field" style={{ marginBottom: 20 }}>
            <label>Working Directory</label>
            <input value={workDir} onChange={e => setWorkDir(e.target.value)} style={{ background: '#1a1a2e', border: '1px solid #2a2a4a', color: '#e0e0e0', padding: 10, borderRadius: 8, fontSize: 14, width: '100%' }} />
          </div>
          ...
          <div className="config-field" style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 6 }}>
              {projects.map(p => (
                <span key={p.dirName} onClick={() => selectProject(p)} style={{
                  background: workDir === p.path ? '#e94560' : '#0f3460',
                  padding: '6px 12px', borderRadius: 16, fontSize: 12, color: '#e0e0e0', cursor: 'pointer'
                }}>{p.name} ({p.sessionCount})</span>
              ))}
            </div>
          </div>
          ...
          <div className="config-field" style={{ marginBottom: 20 }}>
            ...
            <input value={proxyUrl} onChange={e => setProxyUrl(e.target.value)} style={{ background: '#1a1a2e', border: '1px solid #2a2a4a', color: '#e0e0e0', padding: 10, borderRadius: 8, fontSize: 14, width: '100%' }} />
          </div>

          <div className="config-field" style={{ marginBottom: 20 }}>
            <label>Auth Token (optional)</label>
            <input value={authToken} onChange={e => setAuthToken(e.target.value)} type="password" placeholder="Leave empty to disable" style={{ background: '#1a1a2e', border: '1px solid #2a2a4a', color: '#e0e0e0', padding: 10, borderRadius: 8, fontSize: 14, width: '100%' }} />
          </div>
          ...
          {msg && (
            <div style={{ marginTop: 12, padding: 10, borderRadius: 8, fontSize: 13,
              ...
              color: msg.includes('Failed') ? '#ff6b6b' : '#4caf50' }}>{msg}</div>
          )}
        </div>
      </div>
    </div>
